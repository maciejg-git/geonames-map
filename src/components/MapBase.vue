		...
		<div class="bg-light">
			...
			<div class="row no-gutters">
				...
				<div class="col-auto">
					...
					<VSidebar 
						 label="Layers" 
						 v-show="isLayerSidebarVisible"
						 :canCompact="false"
						 >
						 <MapSidebarLayers 
							 :items="layers.hotsprings"
							 @layer-hotsprings-clicked="handleLayerHotspringsClicked"
							 @click-hotspring="handleClickHotspring"
							 @open-context="openContextHotsprings"
							 @mouseenter="handleMouseenterHotspring"
							 @mouseleave="handleMouseleaveHotspring"
							 />
					</VSidebar>
...
import { hotsprings } from "../hotsprings.js"
...
import MapSidebarLayers from "./MapSidebarLayers.vue"
...
export default {
	...
	methods: {
		...
		findHotspringByMarker(marker) {
			return this.layers.hotsprings.find(f => f.marker == marker);
		},

		// LAYERS

		loadLayers() {
			this.layers.hotsprings = Object.freeze(
				Papa.parse(hotsprings.items, { 
					header: true,
					dynamicTyping: true,
				}).data
			);
			this.addLayerHotsprings()
		},
		addLayerHotsprings() {
			if (map.layerHotsprings) {
				map.layerHotsprings.clearLayers();
			}

			let hotsprings = [];

			for (let i of this.layers.hotsprings) {
				let popup = `<b>${i.name || "Unknown"}</b>`

				let marker = L.marker([i.lat, i.lon], {
					icon: LeafletIcons['waves']
				...
				marker.on("contextmenu", e => {
					let hotspring = this.findHotspringByMarker(e.target);
					this.$refs.contextHotsprings.open(e.originalEvent, hotspring);
				});

				i.marker = marker;
				hotsprings.push(marker);
			}

			map.layerHotsprings = L.layerGroup(hotsprings);

			if (this.$store.state.isLayerHotspringsVisible) {
				map.layerHotsprings.addTo(map.map);
			}
		},
		handleLayerHotspringsClicked() {
			if (this.$store.state.isLayerHotspringsVisible) {
				map.layerHotsprings.addTo(map.map);
			}
			else {
				map.layerHotsprings.remove();
			}
		},
		handleMouseenterHotspring(hotspring) {
			let latlng = hotspring.marker.getLatLng();
			if(map.map.getBounds().contains(latlng)) {
				hotspring.marker.openPopup();
			}
		},
		handleMouseleaveHotspring(hotspring) {
			hotspring.marker.closePopup();
		},
		handleClickHotspring(hotspring) {
			map.map.setView([hotspring.lat, hotspring.lon], 14);
		},
